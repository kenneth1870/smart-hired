# How to provision a new environment for smart-hired
...
* Enter webroot path: `/home/deployer/apps/smart_hired/shared/public/`

The `.pem` files created by `certbot-auto` need to be linked so Nginx can see them:

```
ln -s /etc/letsencrypt/live/example.com/fullchain.pem /etc/ssl/smart_hired.crt
ln -s /etc/letsencrypt/live/example.com/privkey.pem /etc/ssl/smart_hired.key
```
...
```
/opt/certbot/certbot-auto renew --webroot-path /home/deployer/apps/smart_hired/shared/public/ --dry-run
```

Finally, add this to the root crontab to enable auto-renewal:

```
51 4,16 * * * /opt/certbot/certbot-auto renew --quiet --no-self-upgrade --webroot-path /home/deployer/apps/smart_hired/shared/public/
```
